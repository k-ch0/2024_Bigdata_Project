...
# '평균최고기온(℃)', '최대순간풍속(m/s)', 'NO2' 컬럼만 추출
df_season = df[['season', '평균최고기온(℃)', '최대순간풍속(m/s)', 'NO2']]
...
# 이미지 저장 폴더 생성
output_dir = 'C:\\Users\\Owner\\Desktop\\2024_kch\\BigData\\project\\regression\\NO2'
...
def plot_seasonal_relation(season_data, season_name):
    ...
    # 평균최고기온과 NO2의 관계 시각화
    plt.subplot(1, 2, 1)
    sns.regplot(x='평균최고기온(℃)', y='NO2', data=season_data, scatter_kws={'s': 50}, line_kws={'color': 'red'})
    corr, p_value = stats.pearsonr(season_data['평균최고기온(℃)'], season_data['NO2'])  # 상관계수, p-value 계산
    slope, intercept, r_value, p_value, std_err = stats.linregress(season_data['평균최고기온(℃)'], season_data['NO2'])  # 기울기 구하기
    plt.title(f'{season_name} - NO2 vs 평균최고기온\n상관계수: {corr:.2f}, 기울기: {slope:.2f}, p-value: {p_value:.4f}', loc='right')

    # y축 범위 확장 (최솟값과 최댓값을 2배로 확장)
    min_y = season_data['NO2'].min()
    max_y = season_data['NO2'].max()
    plt.ylim(min_y - (max_y - min_y) * 1.0, max_y + (max_y - min_y) * 1.0)  # y축 범위 2배 확장

    # 최대순간풍속과 NO2의 관계 시각화
    plt.subplot(1, 2, 2)
    sns.regplot(x='최대순간풍속(m/s)', y='NO2', data=season_data, scatter_kws={'s': 50}, line_kws={'color': 'blue'})
    corr, p_value = stats.pearsonr(season_data['최대순간풍속(m/s)'], season_data['NO2'])  # 상관계수, p-value 계산
    slope, intercept, r_value, p_value, std_err = stats.linregress(season_data['최대순간풍속(m/s)'], season_data['NO2'])  # 기울기 구하기
    plt.title(f'{season_name} - NO2 vs 최대순간풍속\n상관계수: {corr:.2f}, 기울기: {slope:.2f}, p-value: {p_value:.4f}', loc='right')
    ...
    # 이미지 저장
    plt.savefig(f'{output_dir}/{season_name}_NO2_relation.png')
    plt.close()
...
def plot_seasonal_corr(season_data, season_name):
    ...
    # 상관 행렬 계산
    corr = season_data[['평균최고기온(℃)', '최대순간풍속(m/s)', 'NO2']].corr()
