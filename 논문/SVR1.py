...
# 데이터 불러오기
data = pd.read_csv('normalized_with_season.csv', encoding='cp949')
...
for season in ['Spring', 'Summer', 'Fall', 'Winter']:
    ...
    for pollutant in pollutants:
        ...
        plt.title(f'{season} - {pollutant_display} 중요도 (SVR)', fontsize=14)
...
for season, season_results in results.items():
    print(f"--- {season} ---")
    for pollutant, importance_list in season_results.items():
        print(f"{pollutant} 중요도:")
        for condition, importance in importance_list:
